...
export const getOrCreateTooltip = (
    ...
    theme: 'dark' | 'light',
    primaryColor: string,
    className: string = 'chart-tooltip'
) => {
    let tooltip = container.select<HTMLDivElement>(`.${className}`);
    ...
    const shadow = `0 4px 12px ${primaryColor}40`; // 25% opacity
    ...
    if (tooltip.empty()) {
        tooltip = container.append('div')
            ...
            .style('transition', 'opacity 0.4s, transform 0.4s ease-out')
...
export const attachTooltipEvents = (
    ...
    const tooltip = getOrCreateTooltip(container, theme, primaryColor, 'distribution-tooltip');
